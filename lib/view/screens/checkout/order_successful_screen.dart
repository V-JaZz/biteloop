...
class OrderSuccessfulScreen extends StatefulWidget {
  ...
  OrderSuccessfulScreen(
      {required this.orderID, required this.status, required this.totalAmount});
...
class _OrderSuccessfulScreenState extends State<OrderSuccessfulScreen> {
  ...
  getScratchCard() async {
    ...
    final responseOrder = await http.get(
        Uri.parse(
            '${AppConstants.BASE_URL}/api/v1/customer/order/track?order_id=${widget.orderID}'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'Authorization': 'Bearer ${token}',
        });
  ...
  applyCoupon(Map applyData) async {
    ...
    final response = await http.get(
      Uri.parse(
          '${AppConstants.BASE_URL}/api/v1/user-scratcher-apply/${applyData['get_id']}'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': 'Bearer ${token}',
      },
                ...
                child: SizedBox(
                    ...
                    child: Column(
                        ...
                        children: [
                          ...
                          (orderController.trackModel?.paymentStatus ==
                                      'paid' &&
                                  orderController.trackModel!.orderAmount! >
                                      50 &&
                                  orderController.trackModel!.orderAmount! <
                                      1000)
                              ? Column(children: [
                                  if (orderController.trackModel?.orderStatus !=
                                      'rejected')
                                    Scratcher(
                                      enabled: false,
                                      brushSize: 50,
                                  ...
                                  Text(
                                      'You earn the scratch card!\nCan redeemed once your\n order is accepted.'
                                          .tr,
                                      textAlign: TextAlign.center,
                                      style: robotoMedium.copyWith(
