...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    if (_isLoggedIn == true) {
      Get.find<SearchPage>().getSuggestedFoods();
    }
    Get.find<SearchPage>().getHistoryList();
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        if (Get.find<SearchPage>().isSearchMode) {
          return true;
        } else {
          Get.find<SearchPage>().setSearchMode(true);
          return false;
        }
      },
      child: Scaffold(
        ...
        body: SafeArea(
          ...
          child: GetBuilder<SearchPage>(builder: (searchController) {
  ...
  void _actionSearch(SearchPage searchPage, bool isSubmit) {
    if (searchPage.isSearchMode || isSubmit) {
      if (_searchController.text.trim().isNotEmpty) {
        searchPage.searchData(_searchController.text.trim());
      } else {
        showCustomSnackBar('search_food_or_restaurant'.tr);
      }
    } else {
      List<double> _prices = [];
      if (!searchPage.isRestaurant) {
        searchPage.allProductList
      ...
      Get.dialog(FilterWidget(
          maxValue: _maxValue, isRestaurant: searchPage.isRestaurant));
    }
  }
}
