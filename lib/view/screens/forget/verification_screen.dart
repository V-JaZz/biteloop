        ...
        child: Center(
          ...
          child: GetBuilder<AuthController>(builder: (authController) {
            return Column(children: [
              Get.find<SplashController>().configModel?.demo == true
                  ? Text(
                      'for_demo_purpose'.tr,
                      style: robotoRegular,
                    )
                        ...
                        onPressed: _seconds < 1
                            ? () {
                                if (widget.fromSignUp) {
                                  authController
                                      .login(
                                          _number.toString(), widget.password)
                                      .then((value) {
                                    if (value.isSuccess) {
                                      _startTimer();
                                      showCustomSnackBar(
                                  ...
                                  authController
                                      .forgetPassword(_number.toString())
                                      .then((value) {
                                    if (value.isSuccess) {
                                      _startTimer();
                                      showCustomSnackBar(
                  ...
                  ? !authController.isLoading
                      ? CustomButton(
                          ...
                          onPressed: () {
                            if (widget.fromSignUp) {
                              authController
                                  .verifyPhone(_number.toString(), widget.token)
                                  .then((value) {
                                if (value.isSuccess) {
                                  showAnimatedDialog(
                                      context,
