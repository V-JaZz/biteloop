...
class CouponSubscribeScreen extends StatefulWidget {
  ...
  const CouponSubscribeScreen({Key? key, required this.fromCheckout})
      : super(key: key);
  @override
...
class _CouponSubscribeScreenState extends State<CouponSubscribeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'Subscription Coupon'.tr),
      body: _isLoggedIn
          ? GetBuilder<CouponController>(builder: (couponController) {
              return couponController.couponSubscribeList != null
                  ? couponController.couponSubscribeList!.length > 0
                      ? RefreshIndicator(
                          onRefresh: () async {
                            await couponController.getCouponSubscribeList();
                            print(
                                'I am in of coupon subscribe --------------------------------');
                          },
                          child: Scrollbar(
                                                ...
                                                Expanded(
                                                  child: Column(
                                                      ...
                                                      children: [
                                                        ...
                                                        Text(
                                                          '${couponController.couponSubscribeList?[index].discount}${couponController.couponSubscribeList?[index].discountType == 'percent' ? '%' : Get.find<SplashController>().configModel?.currencySymbol} off',
                                                          style: robotoMedium
                                                              .copyWith(
                                                                  color: Theme.of(
                                                                          context)
                                                        ...
                                                        Row(children: [
                                                          ...
                                                          Flexible(
                                                              child: Text(
                                                            '${couponController.couponSubscribeList?[index].couponType?.tr}' +
                                                                '${couponController.couponSubscribeList?[index].couponType == 'restaurant_wise' ? ' (${couponController.couponSubscribeList?[index].data})' : ''}',
                                                            style: robotoMedium.copyWith(
                                                                color: Theme.of(
                                                                        context)
                                                                    .cardColor,
