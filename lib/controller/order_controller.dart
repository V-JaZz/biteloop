...
class OrderController extends GetxController implements GetxService {
  ...
  void canActiveOrder() {
    if (_runningOrderList != null && _runningOrderList!.isNotEmpty) {
  ...
  Future<void> placeSubscribeOrder(PlaceSubscribeBody placeSubscribeBody,
      Function callback, double amount) async {
    _isLoading = true;
    update();
    print('JSON PRINT');
    print(placeSubscribeBody.toJson());
    Response response = await orderRepo.placeSubscribeOrder(placeSubscribeBody);
    _isLoading = false;
    // print('API CALL RESPONSE_--------------------------------------------');
    // print(response.body);
    // print('API CALL RESPONSE2_--------------------------------------------');
    if (response.statusCode == 200) {
      String message = response.body['message'];
      String orderID = response.body['order_id'].toString();
      callback(true, message, orderID, amount);
