...
class CouponController extends GetxController implements GetxService {
  ...
  List<CouponModel>? _couponSubscribeList;
  CouponModel? _coupon;
  ...
  List<CouponModel>? get couponSubscribeList => _couponSubscribeList;
  ...
  Future<void> getCouponSubscribeList() async {
    Response response = await couponRepo.getCouponSubscribeList();
    if (response.statusCode == 200) {
      _couponSubscribeList = [];
      response.body.forEach((category) =>
          _couponSubscribeList?.add(CouponModel.fromJson(category)));
      update();
    } else {
      ApiChecker.checkApi(response);
    }
